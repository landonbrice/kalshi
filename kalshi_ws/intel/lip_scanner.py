...
def write_candidates_csv(path: Path, candidates: list[LipCandidate]) -> None:
    ...
    fields = [
        ...
        "mid",
        "volume_24h",
        # Market-quality signals (float precision from Kalshi `_fp` fields).
        "volume_total",
        "open_interest",
        "liquidity_dollars",
        "yes_bid_cents",
            ...
            w.writerow(
                {
                    ...
                    "volume_total": f"{c.market.volume_fp:.2f}",
                    "open_interest": f"{c.market.open_interest_fp:.2f}",
                    "liquidity_dollars": f"{c.market.liquidity_dollars:.4f}",
